# Architecture Design: A Modular Pipeline for FHIR-Based Clinical Data Analysis
...
This document describes the implementation design of the MIMIC-FHIR analytics pipeline, focusing on the concrete architectural patterns, technology integration, and naming conventions that enable modular, iterative clinical data analysis. The pipeline implements a medallion architecture (Bronze-Silver-Gold) within a single DuckDB database, with ingestion and transformation separated into independent, containerized workflows. We detail the role of each technology—FHIR/NDJSON for interoperable data ingestion, DuckDB as a unified analytical storage engine, SQL-based flatten views for silver-layer extraction, SQLMesh for gold-layer orchestration, and Docker for reproducible execution. The design prioritizes a run-once ingestion model with iterate-often transformations, scalable naming conventions, and memory-aware execution for constrained environments.
...
### 3.2. DuckDB as Unified Analytical Storage
